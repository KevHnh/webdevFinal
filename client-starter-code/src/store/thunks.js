...
export const fetchAllCampusesThunk = () => async (dispatch) => {  // The THUNK
  try {
    ...
    let res = await axios.get(`http://localhost:9000/api/campuses`);  
...
export const fetchCampusThunk = (id) => async (dispatch) => {  // The THUNK
  try {
    ...
    let res = await axios.get(`http://localhost:9000/api/campuses/${id}`);  
...
export const addCampusThunk = (campus) => async (dispatch) => {  // The THUNK
  try {
    ...
    let res = await axios.post(`http://localhost:9000/api/campuses`, campus);  
...
export const deleteCampusThunk = campusId => async dispatch => {  // The THUNK
  try {
    ...
    await axios.delete(`http://localhost:9000/api/campuses/${campusId}`);  
...
export const editCampusThunk = campus => async dispatch => {  // The THUNK
  console.log("Shamim Rahman")
  console.log("campus")
  try {
    // API "put" call to update student (based on "id" and "student" object's data) from database
    let updatedCampus = await axios.put(`http://localhost:9000/api/campuses/${campus.id}`, campus); 
...
export const fetchAllStudentsThunk = () => async (dispatch) => {  // The THUNK
  try {
    ...
    let res = await axios.get(`http://localhost:9000/api/students`);  
...
export const addStudentThunk = (student) => async (dispatch) => {  // The THUNK
  ...
  for (let i = 0; i< allCamp.data.length; i++) {
    console.log(allCamp.data[i].id)
    console.log(student.campusId)

    if (student.campusId === allCamp.data[i].id || student.campusId === null) {
      res = await axios.post(`http://localhost:9000/api/students`, student);
...
export const deleteStudentThunk = studentId => async dispatch => {  // The THUNK
  try {
    // API "delete" call to delete student (based on "studentID") from database
    await axios.delete(`http://localhost:9000/api/students/${studentId}`);  
    // Delete successful so change state with dispatch
...
export const editStudentThunk = student => async dispatch => {  // The THUNK
  try {
    // API "put" call to update student (based on "id" and "student" object's data) from database
    let updatedStudent = await axios.put(`http://localhost:9000/api/students/${student.id}`, student); 
    // Update successful so change state with dispatch
...
export const fetchStudentThunk = id => async dispatch => {  // The THUNK
  try {
    ...
    let res = await axios.get(`http://localhost:9000/api/students/${id}`);  
